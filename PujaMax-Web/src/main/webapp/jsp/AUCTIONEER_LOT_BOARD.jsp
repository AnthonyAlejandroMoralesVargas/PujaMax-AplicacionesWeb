...
<head>
	...
	<title>Auctioneer Lot Board</title>
	<!-- Bootstrap CSS -->
	<link
	...
	<main class="container my-4">
		<section class="home-container">
			<nav class="nav-container">
				<a
					href="${pageContext.request.contextPath}/LotManagementController?route=list"
			...
			<div class="row">
				...
				<c:forEach var="lot" items="${lots}">
					<div class="col-md-6 col-lg-6 mb-4">
						<!-- Clase condicional -->
						<div
							class="card ${lot.state == 'INACTIVE' ? 'inactive-card' : ''}">
							<!-- Cabecera de la tarjeta -->
							<div class="card-header">
								<c:choose>
									<c:when test="${lot.state == 'ACTIVE'}">
										<span class="badge bg-success p-2"> <i
											class="fas fa-gavel"></i> ACTIVE
										</span>
									</c:when>
									<c:otherwise>
										<span class="badge bg-secondary p-2"> <i
											class="fas fa-gavel"></i> INACTIVE
										</span>
									</c:otherwise>
								</c:choose>
								<!-- Enlace a detalles -->
								<a
									href="${pageContext.request.contextPath}/ProductManagementController?route=list&idLot=${lot.idLot}"
									class="text-white" title="Go to Details"> <i
							...
							<div
								...
								<div class="action-icons d-flex flex-column align-items-center">
									<!-- EDITAR -->
									<a
										href="${pageContext.request.contextPath}/LotManagementController?route=edit&idLot=${lot.idLot}"
										class="nav-item text-primary" title="Edit"> <i
										class="fas fa-edit"></i>
									</a>
									<!-- ELIMINAR -->
									<a
										href="${pageContext.request.contextPath}/LotManagementController?route=delete&idLot=${lot.idLot}"
							...
							<c:set var="currentDateMillis"
								value="<%=currentDate.getTime()%>" />
							<c:set var="dateClosingMillis" value="${lot.dateClosing.time}" />
							<c:set var="diffInMillis"
								value="${dateClosingMillis - currentDateMillis}" />
							<c:set var="hoursRemaining"
								value="${diffInMillis / (60 * 60 * 1000)}" />
							...
							<div class="stats row text-center">
								<div class="stat col-6 border">
									...
									<p>
										<i class="fas fa-box"></i> Products in Auction
									</p>
								</div>
								<div class="stat col-6 border">
									<h3>${roundedHoursRemaining}</h3>
									<p>
										<i class="fas fa-clock"></i> Hours to Close
									</p>
								</div>
							</div>
						</div>
		...
		<c:if test="${empty lots}">
			<div class="alert alert-warning text-center">You have not created any lot.</div>
		</c:if>
	</main>
	<!-- Footer -->
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form
						action="${pageContext.request.contextPath}/LotManagementController?route=saveNew"
						method="POST">
						<div class="mb-3">
							<input type="hidden" name="txtId" id="txtId"> <label
								for="txtTitle" class="form-label">Title</label> <input
								type="text" class="form-control" id="txtTitle" name="txtTitle"
								placeholder="Enter title"> <input type="hidden"
								name="txtQuantityProducts" id="txtQuantityProducts" value="0">
						</div>
						<div class="mb-3">
							<label for="txtOpeningDate" class="form-label">Opening
								Date</label> <input type="date" class="form-control" id="txtOpeningDate"
								name="txtOpeningDate">
						</div>
						<div class="mb-3">
							<label for="txtClosingDate" class="form-label">Closing
								Date</label> <input type="date" class="form-control" id="txtClosingDate"
								name="txtClosingDate">
						</div>
						<div class="mb-3">
							<label for="delivery" class="form-label">Address</label> <select
								id="delivery" class="form-select" name="txtIdAddress">
	...
	<div class="modal fade" id="EDIT_LOT_FORM" tabindex="-1"
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<form
					action="${pageContext.request.contextPath}/LotManagementController?route=saveExisting"
					method="POST">
					<div class="modal-body">
						<!-- Hidden ID Field -->
						<input type="hidden" name="txtId" id="editLotId"
							value="${lot.idLot}">

						<!-- Title -->
						<div class="mb-3">
							<label for="editLotTitle" class="form-label">Title</label> <input
								type="text" class="form-control" name="txtTitle"
								id="editLotTitle" value="${lot.title}" required> <input
								type="hidden" name="txtQuantityProducts"
								id="editLotQuantityProducts" value="${lot.quantityProducts}">
						</div>

						<!-- Opening Date -->
						<div class="mb-3">
							<label for="editLotOpeningDate" class="form-label">Opening
								Date</label> <input type="date" class="form-control"
								name="txtOpeningDate" id="editLotOpeningDate"
								value="<fmt:formatDate value='${lot.dateOpening}' pattern='yyyy-MM-dd' />"
								required>
						</div>

						<!-- Closing Date -->
						<div class="mb-3">
							<label for="editLotClosingDate" class="form-label">Closing
								Date</label> <input type="date" class="form-control"
								name="txtClosingDate" id="editLotClosingDate"
								value="<fmt:formatDate value='${lot.dateClosing}' pattern='yyyy-MM-dd' />"
								required>
						</div>

						<!-- Address Dropdown -->
						<div class="mb-3">
							<label for="editLotAddress" class="form-label">Address</label> <select
								class="form-select" name="txtIdAddress" id="editLotAddress"
								required>
				...
				<div class="modal-body text-center">
					...
					<div class="modal-body text-center">
						...
						<p class="card-text text-secondary small mb-4">
							<i class="fas fa-map-marker-alt me-2"></i>${lot.address.province},
							${lot.address.city}
						</p>
						<p class="card-text text-secondary small mb-4">
