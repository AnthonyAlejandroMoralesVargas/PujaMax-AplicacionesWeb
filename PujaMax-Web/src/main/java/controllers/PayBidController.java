...
public class PayBidController extends HttpServlet {
	...
	private void viewHistory(HttpServletRequest request, HttpServletResponse response)
		...
		// Utilizar el servicio para obtener las pujas
		int bidderId = bidder.getId();
		BidService bidService = new BidService(); // Instancia del servicio
		List<Bid> bids = bidService.getBidsByUserId(bidderId); // Llamar al método del servicio
	...
	private void payWinningBid(HttpServletRequest request, HttpServletResponse response)
		...
		String bidIdParam = request.getParameter("idBid");
		...
		try {
			...
			Bid.BidState newState = Bid.BidState.valueOf(stateParam); // Convertir el parámetro a enum

			// Usar el servicio para actualizar el estado de la puja
			BidService bidService = new BidService();
			boolean success = bidService.updateBidState(bidId, newState);

			if (!success) {
				request.setAttribute("messageType", "error");
				request.setAttribute("message", "Could not update bid state.");
			}

		} catch (IllegalArgumentException e) {
			// Manejar errores en la conversión del ID o del estado
			request.setAttribute("messageType", "error");
			request.setAttribute("message", "Invalid bid ID or state value.");
		}
